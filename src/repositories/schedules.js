...
async function insertSchedules(scheduling) {
  ...
  const [rows] = await conn.query(
    "INSERT INTO scheduling(send_date, receiver, message, message_type) VALUES (?, ?, ?, ?)",
    [
      scheduling.send_date,
      scheduling.receiver,
      scheduling.message,
      scheduling.message_type,
    ]
  );
  return rows;
}
...
async function deleteSchedules(id) {
  const conn = await connect();
  const [rows] = await conn.query("DELETE FROM scheduling WHERE id=?;", id);
  return rows;
}

module.exports = { selectSchedules, selectSchedulesById, insertSchedules, deleteSchedules };
